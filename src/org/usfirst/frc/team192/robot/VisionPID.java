...
public class VisionPID implements PIDOutput, PIDSource {
	...
	public void pidWrite(double output) {
		...
		/*
		 * if (changeoutput) { if (Math.abs(camerror) < 50) { runforward = true; }else {
		 * runforward = false; }
		 * 
		 * if (camerror < 0) { if (runforward){ swerve.setWithAngularVelocity(.7, .5,
		 * 0); }else { swerve.setWithAngularVelocity(.7, .5, 0); } } else if (camerror >
		 * 0) { if (runforward){ swerve.setWithAngularVelocity(.7, -.5, 0); }else {
	...
	public void update() {
		...
		// angle_pid.setSetpoint(SmartDashboard.getNumber("blockLocation",
		// CAMCENTER.x));
